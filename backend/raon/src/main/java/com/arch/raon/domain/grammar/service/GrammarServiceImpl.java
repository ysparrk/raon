...
//	}
	...
	@Override
	public List<GrammarMyRankQueryDTO> getMiddleRankResult(Long myRank, List<GrammarRankQueryDTO> allRanks) {
		List<GrammarMyRankQueryDTO> selectedRankResults = new ArrayList<>();

		// TOP3
		for (Long i = 0L; i < 3; i++) {
			if (i < allRanks.size()) {
				GrammarRankQueryDTO baseDTO = allRanks.get(i.intValue());
				Member member = memberRepository.findById(baseDTO.getMemberId()).orElseThrow(() -> new CustomException(ErrorCode.MEMBER_NOT_FOUND) {
					@Override
					public ErrorCode getErrorCode() {
						return super.getErrorCode();
					}
				});
				selectedRankResults.add(new GrammarMyRankQueryDTO(i+1, member.getNickname(), baseDTO.getScore()));
			}

		}

		// member의 바로 위 한명, member, member의 바로 아래 한명
		for (Long i = myRank - 1; i <= myRank + 1; i++) {
			if (i >= 0 && i < allRanks.size()) {
				GrammarRankQueryDTO baseDTO = allRanks.get(i.intValue());
				Member member = memberRepository.findById(baseDTO.getMemberId()).orElseThrow(() -> new CustomException(ErrorCode.MEMBER_NOT_FOUND) {
					@Override
	...
	@Override
	public List<GrammarMyRankQueryDTO> getTopRankResult(List<GrammarRankQueryDTO> allRanks) {
		List<GrammarMyRankQueryDTO> selectedRankResults = new ArrayList<>();

		// TOP6
		for (int i = 0; i < 6; i++) {
			if (i < allRanks.size()) {
				GrammarRankQueryDTO baseDTO = allRanks.get(i);
				Member member = memberRepository.findById(baseDTO.getMemberId()).orElseThrow(() -> new CustomException(ErrorCode.MEMBER_NOT_FOUND) {
					@Override
					public ErrorCode getErrorCode() {
						return super.getErrorCode();
	...
	@Override
	public List<GrammarMyRankQueryDTO> getLastRankResult(List<GrammarRankQueryDTO> allByCountry) {
		List<GrammarMyRankQueryDTO> selectedRankResults = new ArrayList<>();
	...
	@Override
	public GrammarMyRankingResDTO getRankByGrammarRanking(Long memberId, List<GrammarMyRankQueryDTO> rankList) {
		Member member = memberRepository.findById(memberId).orElseThrow(() -> new CustomException(ErrorCode.MEMBER_NOT_FOUND) {
			@Override
			public ErrorCode getErrorCode() {
				return super.getErrorCode();
		...
		if (myIdx < 5) {
			// TOP_PLACE
			List<GrammarMyRankQueryDTO> topPlaceRankResult = getTopRankResult(rankList);
			...
			// LAST_PLACE
			List<GrammarMyRankQueryDTO> lastPlaceRankResult = getLastRankResult(rankList);
			...
			// MIDDLE_PLACE
			List<GrammarMyRankQueryDTO> middlePlaceRankResult = getMiddleRankResult(myIdx, rankList);
	...
	public GrammarMyRankingResDTO getRank(Long memberId, GrammarRanking grammarRanking) {
		...
		switch (grammarRanking) {
			case GRAMMAR_COUNTRY_MY -> {
				Long myRank = redisService.getMyCountryGrammarRank(memberId);
				List<GrammarRankQueryDTO> countryGrammarRankList = redisService.getCountryGrammarRank();
